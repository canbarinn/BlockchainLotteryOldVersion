...
contract DALottery {
    ...
    function getTicket(uint lottery_no) public view returns (TicketTier) {
        return tickets[msg.sender][lottery_no][0].ticketTier;
    }

    function makeTicketPayment(bytes32 hash_rand_number, uint amount) public {
        //NOTE: ticket tier enum olmali :()
        require(balance[msg.sender] >= amount, "insufficient balance");
        balance[msg.sender] = balance[msg.sender] - amount;
        buyTicket(hash_rand_number);
        if (amount == 8) {
            tickets[msg.sender][lotteryNoCounter][
                tickets[msg.sender][lotteryNoCounter].length - 1
            ...
            tickets[msg.sender][lotteryNoCounter][
                tickets[msg.sender][lotteryNoCounter].length - 1
            ].ticketTier = TicketTier.Invalid;
        }

        lotteryInfos[lotteryNoCounter].lotteryBalance += amount;
    }
